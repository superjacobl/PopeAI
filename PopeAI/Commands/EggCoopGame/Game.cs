...
public static class GameData
{
    ...
    public static List<Research> ResearchData = new() {
        ...
        new Research("Improved Genetics", 5, 100, (x => Math.Pow(1.375, Math.Min(50, x)) * Math.Pow(1.45, Math.Max(0, x-50)) * 4_500_000_000_000), new() { new(ModifierType.EggValue, 0.15), new(ModifierType.EggLayingRate, 0.15) }, 12, "Increase egg value AND egg laying rate by 15%"),
        ...
        new Research("Graviton Coating", 7, 7, (x => Math.Pow(300, x) * 70_000_000_000_000_000_000_000_000_000.0), new() { new(ModifierType.EggValue, 1) }, 19, "Doubles egg value", EffectType.Multiplicative),
        new Research("Machine Learning Incubators", 7, 250, (x => Math.Pow(1.1, x) * 1.5e23), new() { new(ModifierType.ChickenProductionPerHab, 5) }, 20, "Each hab produces 5 more chickens per minute"),
        new Research("Crystalline Shelling", 7, 100, (x => Math.Pow(1.31, x) * 400_000_000_000_000_000_000_000_000_000.0), new() { new(ModifierType.EggValue, 0.25) }, 21, "Increase egg value by 25%"),
    };
    ...
    public static List<Boost> BoostData = new()
    {
        ...
        new Boost(6, "", "Large Tachyon Prism", "10x internal hatchery for 6hr", new() { new(ModifierType.ChickenProductionPerHabFactor, 9) }, TimeSpan.FromHours(6), 500),
    ...
    public static double GetTotalEffectForType(UserEggCoopGameData player, ModifierType modifierType, bool isadditive = false, bool includeboosts = true)
    {
        ...
        if (includeboosts)
        {
            ...
            if (boosts.Count > 0)
            {
                ...
                if (!isadditive)
                    total *= (firstboost.Modifiers.First(x => x.Type == modifierType).Value * boosts.Count) + 1;
                else
                    total += (firstboost.Modifiers.First(x => x.Type == modifierType).Value * boosts.Count);
            }
        }
